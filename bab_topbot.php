...
Class better_Author_box_red {
	...
	public static function init() {
		global $wp_version;
		// better Author Bio requires Wordpress 2.9 or grater
		...
		if( is_single() ) {
			...
			<div id="better-author-bio-div">
				<div class="better-author-bio-div-info">
					...
					<p class="better-author-bio-div-text"><?php echo esc_attr(sprintf(__ngettext('%s %s %d %s', '%s %s %d %s', $author['posts'], $bab_auttxt1, $bab_auttxt2, 'better-author-bio-div'), get_the_author_firstname().' '.get_the_author_lastname(), $bab_auttxt1, $author['posts'], $bab_auttxt2 )); ?>.</p>
					<?php } else echo "<br />"; ?>
...
<li><a href="<?php echo $author['lnkin']; ?>"  title="<?php echo esc_attr(sprintf(__('Connect with %s', 'better-author-bio-div'), $author['name'])); ?>" rel="external">LinkedIn</a></li>
  ...
  if(!better_Author_box_red::init()) {
	echo 'better-author-bio-div plugin requires WordPress 2.9 or higher. Please upgrade!';
}
